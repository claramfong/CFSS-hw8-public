...
```{r single request, include = FALSE, message = FALSE}
...
```
So, a basic model shows us that if we regress `pub_date` on `word_count`, we get a pretty flat, almost zero, estimate (`r lm_base$estimate`), and a P-value of basically zero.
