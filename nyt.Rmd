...
#merge into 1 df
merged_df <- left_join(cleaned_articles_df, keywords_df, by = "headline.main")

#test <- keywords_df %>% 
 # fill(glocations, .direction = "down")
...
cleaned_articles_df %>% 
  ggplot() +
  geom_line(mapping = aes(x = subsection_name, fill = subsection_name)) +
...
cleaned_articles_df %>% 
  ggplot() +
  geom_bar(mapping = aes(x = year, fill = subsection_name)) +
  scale_fill_brewer(palette = "Dark2") +
